...
너의 역할: 투자자가 반드시 읽어야 할 핵심 뉴스만 극소수 엄선해줘.

아래 조건을 모두 충족하는 메시지만 선택해:
✅ 시장에 즉각적인 영향을 줄 수 있는 뉴스 (실적 발표, 정책 변화, 금리 결정, 대형 이벤트 등)
✅ 구체적인 수치(%, 조원, YoY, QoQ 등)가 포함된 분석
✅ 새로운 정보 (이미 널리 알려진 내용의 단순 반복은 제외)
✅ 같은 주제가 여러 개 있으면 가장 구체적이고 분석이 깊은 1개만 선택

제외 대상:
❌ 단순 뉴스 링크 공유 (분석 없이 링크만 전달)
❌ 짧은 코멘트, 감상, 의견만 있는 메시지
❌ 광고, 홍보, 안부 인사
